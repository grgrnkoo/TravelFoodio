...
export default function FeedbackPage() {
    ...
    const [publicFeedback, setPublicFeedback] = useState<IFeedback[]>([]);
    ...
    const sender = userProfileDynamic?.email;
    ...
    const handleSendFeedback = async () => {
        ...
        setIsLoading(true);
        try {
            const result = await sendFeedback(textareaValue, sender || null, showPopup, setTextareaValue, !isPrivate);
    ...
    const handleOnChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => setTextareaValue(e.target.value);
                    ...
                    <>
                        <div className="space-y-4">
                            {publicFeedback.map((feedback, index) => (
                                <FeedbackItem key={`${feedback.id || index}-${index}`} feedback={feedback} />
                            ))}
                        </div>
