...
interface DislikedMealsListProps {
    meals: IMeal[];
}

const MEALS_PER_PAGE = 10;

export default function DislikedMealsList({ meals }: DislikedMealsListProps) {
    const [displayCount, setDisplayCount] = useState(MEALS_PER_PAGE);

    if (meals.length === 0) {
        return (
            <div className="w-full">
                <p className="text-center text-muted-foreground py-8">You haven't added any disliked meals yet.</p>
            </div>
        );
    }
