...
const MenuDishSkeleton = ({ showLike, className }: { showLike?: boolean; className?: string }) => {
  return (
    <>
      <style>{`
      @keyframes dropIn {
  ...
  `}</style>
      ...
      transition-all duration-300 my-4 animate-drop-in relative hover:cursor-default ${className}
  `}
        style={{
          transform: 'translateY(-25%)',
          opacity: .5,
